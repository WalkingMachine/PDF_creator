...
int main (int argc, char **argv){
	...
	dirp = opendir("/tmp/snpsht_tmp/snapshots"); /* There should be error handling after this */
	...
	fHandle = fopen("/tmp/snpsht_tmp/pdf_config.conf","w");
	if(fHandle != NULL){
		printf("File created\n");
	} else {
	...
	fprintf(fHandle,"/tmp/sam_temp\n\n");
	fprintf(fHandle,"1 \n\n");
	fprintf(fHandle,"1 \n\n");
	fprintf(fHandle,"The STAGE\n\n");

	if(file_count > 0){
		fprintf(fHandle,"/tmp/snpsht_tmp/snapshots/camera.png\n");
		fprintf(fHandle,"Screenshot : 1 \n");
		if(file_count > 1){
			while(i<file_count){
				fprintf(fHandle,"/tmp/snpsht_tmp/snapshots/camera-%d.png\n",i);
